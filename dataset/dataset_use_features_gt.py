...
class RIODatasetSceneGraph(data.Dataset):
    ...
    def read_relationship_json(self, json_file, box_json_file):
        ...
        with open(json_file, "r") as read_file:
            ...
            for scene in data['scenes']:
                ...
                if 'goal' in scene["scene_id"]:
                    self.scans.append(scene["scene_id"]+'_1')
                self.scans.append(scene["scene_id"]+'_2') #mid只有一个视角
    ...
    def __getitem__(self, index):
        ...
        view_index = scene_id.split('_')[-1]
        scene_id = scene_id[:-2] #去掉view_index
        ...
        if (self.with_feats and (not os.path.exists(feats_path) or self.recompute_feats)) or self.use_points:
            if file in self.files: # Caching
                ...
                points, instances = util.get_partial_point_cloud_of_initial_from_goal(file, labelName2InstanceId, view_index)#! points: 所有的vertices, instances: 对应的instance_id(global_id)
                #! 这里的points已经归一化
