...
class Sg2ScVAEModel(nn.Module):
    ...
    def manipulate(self, z, objs, triples):
    ...
    def decoder(self, z, objs, triples):
        s, p, o = triples.chunk(3, dim=1)  # All have shape (T, 1)
        s, p, o = [x.squeeze(1) for x in [s, p, o]]  # Now have shape (T,)
        edges = torch.stack([s, o], dim=1)  # Shape is (T, 2)
        ...
        if self.decoder_cat:#! 进入
            if self.dist_before:
                #print(f"{z.shape}==(n_obj, 256), {obj_vecs_box.shape}, {obj_vecs_shape.shape}") torch.Size([25, 256])==(n_obj, 256), torch.Size([25, 256]), torch.Size([25, 256])
                obj_vecs_box = torch.cat([obj_vecs_box, z], dim=1)
                obj_vecs_shape = torch.cat([obj_vecs_shape, z], dim=1)
                #print(f"After updating {obj_vecs_box.shape}, {obj_vecs_shape.shape}") After updating torch.Size([25, 512]), torch.Size([25, 512])
            #print(f"{pred_vecs_box.shape}, {pred_vecs_shape.shape}") torch.Size([81, 512]), torch.Size([81, 512])
            ...
            #print(f"after GCN {pred_vecs_box.shape}, {pred_vecs_shape.shape}") after GCN torch.Size([81, 512]), torch.Size([81, 512])
        else:
        ...
        #print(f"{boxes_pred.shape}, {shapes_pred.shape}") torch.Size([25, 6]), torch.Size([25, 128])
        
        return boxes_pred, shapes_pred

    def decoder_with_changes(self, z, dec_objs, dec_triples, missing_nodes, manipulated_nodes):
        # append zero nodes
        nodes_added = []
        ...
        z_prime = self.manipulate(z_prime, dec_objs, dec_triples)
        ...
        dec_man_enc_boxes_pred, dec_man_enc_shapes_pred = self.decoder(z, dec_objs, dec_triples)
    ...
    def forward(self, enc_objs, enc_triples, enc_boxes, enc_shapes, enc_objs_to_scene, \
                dec_objs, dec_triples, dec_boxes, dec_shapes, dec_objs_to_scene, missing_nodes, manipulated_nodes):
        ...
        if self.with_changes:
            ...
            z_prime = torch.cat([z, change_repr], dim=1)  # [25,512]
        ...
        dec_man_enc_boxes_pred, dec_man_enc_shapes_pred = self.decoder(z, dec_objs, dec_triples)#! 预测结果
    ...
    def sample(self, point_ae, mean_est, cov_est, dec_objs,  dec_triples):#! 它从已经训练好的模型中生成样本。
        with torch.no_grad():
            ...
            boxes_pred, shapes_pred = self.decoder(z, dec_objs, dec_triples)
