...
class VAE(nn.Module):
    ...
    def forward_mani(self, enc_objs, enc_triples, enc_boxes, enc_shapes, enc_objs_to_scene, dec_objs,
                     dec_triples, dec_boxes, dec_shapes, dec_objs_to_scene, missing_nodes,
                     manipulated_nodes):
        ...
        if self.type_ == 'shared':
            mu, logvar, orig_gt_boxes, orig_gt_shapes, orig_boxes, orig_shapes, boxes, shapes, keep = \
                self.vae.forward(enc_objs, enc_triples, enc_boxes, enc_shapes, enc_objs_to_scene,
                                 dec_objs, dec_triples, dec_boxes, dec_shapes, dec_objs_to_scene,
    ...
    def decoder_with_changes_boxes_and_shape(self, z_box, objs, triples, missing_nodes, manipulated_nodes, atlas):
        if self.type_ == 'shared':
            boxes, feats, keep = self.vae.decoder_with_changes(z_box, objs, triples, missing_nodes, manipulated_nodes)
    ...
    def sample_box_and_shape(self, point_ae, dec_objs, dec_triplets):
        if self.type_ == 'shared':#! 进入
            return self.vae.sample(point_ae, self.mean_est, self.cov_est, dec_objs,  dec_triplets)
